...
%% Cross-Validation over cells
k = (1:14)*3-2;
nfold = 3;
cv_results_test = zeros(size(k,2),1);
cv_results_train = zeros(size(k,2),1);
for i = 1:size(k,2)
    [cv_results_test(i), cv_results_train(i)] = cv_knn(X, D, L, nfold, k(i));
end
% cv_results
plot(k,cv_results_test)
hold on
plot(k,cv_results_train)
hold off
